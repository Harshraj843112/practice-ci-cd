            ...
            steps {
                script {
                    sh '''#!/bin/bash
                        ...
                        # Install dependencies (no --cache to avoid space issue)
                        npm install --no-audit --no-fund --omit=dev --verbose
                        ...
                        if [ ! -f node_modules/react-scripts/bin/react-scripts.js ]; then
                            ...
                            npm install react-scripts@5.0.1 --no-audit --no-fund --omit=dev --verbose
                        fi
